...
def main():
    ...
    for case in cases:
        ...
        with patch.object(backend, "load_store", return_value=store), \
             patch.object(backend, "ensure_template", return_value=case["template"]), \
            ...
            backend.use_bundled_template(SimpleNamespace(template="synthetic-template"), case["preset"], case["keepMode"])
