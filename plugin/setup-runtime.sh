...
release_fields() {
  ...
  jq -er --arg arch "$arch" '
    ...
    | select(.version == "0.8.2" and (.packages | type) == "object")
...
setup_main() {
  ...
  (
    trap 'rm -f -- "$setup_temp/omavless-0.8.2-1-aarch64.pkg.tar.zst" "$setup_temp/omavless-0.8.2-1-x86_64.pkg.tar.zst"; rmdir -- "$setup_temp" "$setup_lock"' EXIT
